...
class XmlParser(object):
    ...
    def get_media_source_from_url(self, url):
        """
        ...
        # Media
        if self.MEDIA_URL and url.startswith(self.MEDIA_URL):
            return os.path.join(self.MEDIA_ROOT, url[len(self.MEDIA_URL):].lstrip('/'))

        # Static
        elif self.STATIC_URL and url.startswith(self.STATIC_URL):
            return find(url[len(self.STATIC_URL):].lstrip('/'))
    ...
    def vector(self, e):
        ...
        fh = open(self.get_media_source_from_url(path), 'rb')
        data = fh.read()
        fh.close()
    ...
    def img(self, e):
        ...
        img_obj = Image(self.get_media_source_from_url(path), width=width, height=height)
    ...
    def import_pdf_font(self, base_name, face_name):
        ...
        if self.fonts.get(face_name, None) is None:
            afm = self.get_media_source_from_url(base_name + '.afm')
            pfb = self.get_media_source_from_url(base_name + '.pfb')
